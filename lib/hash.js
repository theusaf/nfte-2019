...
module.exports = function(password,created){
  const salt = "BE_PREPARED_FOR_CHANGE";
  const hash = new sha512();
  const date = created ? created : Date.now();
  hash.update(JSON.stringify(matrixEncode(password)) + salt + md5(btoa(password) + date),"utf8");
  return {
    created: date,
    hash: hash.digest("hex")
  }
}
// SHA3 [ JSON [ MATRIX ENCODE [ PASS ] ] + SALT + MD5 [ BASE64 [ PASS ] + Date ] ]
